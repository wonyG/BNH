	...
	<div class="container">
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				<thead>
					<tr>
						...
						<th style="background-color: #eeeeee; text-align: center;">In/Out</th>
				...
				<tbody>
					<%					
						...
						ArrayList<Household> list = householdDAO.getList(pageNumber);
					...
					%>
					<tr>
						<td><%= list.get(i).getHouseholdDate().substring(0,16) %></td>
						<td><% 
								if(list.get(i).getHouseholdInOut() == 1){ 
							%> 
								Income
							<%
								} else {
						...
						<td><a href="viewHousehold.jsp?householdID=<%= list.get(i).getHouseholdID()%>"><%= list.get(i).getHouseholdContent() %></a></td>
						<td><%= list.get(i).getHouseholdPrice() %></td>						
			...
			<%
				if(pageNumber != 1){
			%>
				<a href="household.jsp?pageNumber=<%=pageNumber - 1 %>" class="btn btn-success btn-array-left">Prev</a>
			<%
				}
				if(householdDAO.nextPage(pageNumber + 1)){
			%>
				<a href="household.jsp?pageNumber=<%=pageNumber + 1 %>" class="btn btn-success btn-array-left">Next</a>
			<%
				}
			%>
			<a href="writeHousehold.jsp" class="btn btn-primary pull-right">Write</a>
		</div>
	</div>
